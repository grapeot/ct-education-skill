...
- [Cinematic preparation/composition](../src/ct_education/cinematic.py), [Blender renderer](../src/ct_education/cinematic_blender.py), and [film design](../docs/video_design.md).
...
Both panels consume one source-plane/image state. Native `[k,j,i]` arrays map through the source affine to RAS millimeters; Blender uses `world_xyz = 0.001 * (ras_xyz - stored_center_ras)`, with no axis permutation or sign flip. Plane pixel centers, crop, window, and orientation remain shared, with superior-up non-axial presentation and fail-closed in-plane shear handling. The shared footprint is a display mask, not reviewed segmentation. Left rulers project true plane-local millimeter endpoints through the current camera; right rulers use the actual resized image rectangle and physical crop extent. Display zoom never adds source resolution.

The pre-28-second imagery is unchanged, including staged stack entry, branch emphasis, the fast-slow-fast turn, sweeps, and stable 26-28 second parity proof. At 28-33 seconds, the left is a true 3D near-top view with existing branches, faint envelope, translucent source plane, and world-space candidate ring, not a duplicate flat CT zoom; the right shows native CT detail. Both settle at 30 seconds. The 33-36 second ending fades left context and locator to airway candidates only, with bones hidden, an approximately 21-degree camera arc, 10% field-width narrowing, and a 35.5-36 second hold. Right CT and its metric ruler remain fixed after 30 seconds; the left ruler disappears with the plane at 33.6 seconds. No supplied candidate means no locator or fabricated finding. Native HU is unchanged; no capillaries or nodules are invented, and locators are not lesion meshes or measurements.

The revised authorized 36-second, 1920x1080, 24 fps film completed with 864 Blender frames, unchanged pre-28-second imagery, no temporal interpolation, and no audio; full validation/decode passed per maintainer confirmation. Output includes `state.json`, copied `generate_scene.py`, textures/meshes, left renders/projection metadata, composed frames, packed `master.blend`, benchmark/logs, `film.mp4`, probe, and validation records. Script plus state, not the packed initial scene alone, define the whole motion. Every artifact remains private; the `.blend` contains patient-derived content.
