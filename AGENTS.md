...
- Preserve the first 28 seconds and the 26-28 second parity proof. The 28-36 second local sequence keeps N1 brackets/leader visible in both panels, a native-HU ROI on the left, and the camera locked to N1 through a 50-degree orbit, not airway-centered framing. Right CT stays fixed after 30 seconds; both qualified rulers persist. Interactive annotation locators are unaffected by this film-only postprocessing.
- Keep strict boundary assessment separate from exploratory display: threshold sensitivity or lower-threshold edge leakage requires `mesh=None` and `report.status=localized_region_boundary_unverified`. A separate `density_mesh` requires a bounded non-edge selected component passing basic checks and the exact runtime native-HU isovalue; it never verifies a nodule boundary. Retain the on-frame disclaimer and warnings. Unsafe selected components use native-HU planes and a wire locator, not fabricated geometry. See [the contract](docs/rfc.md#cinematic-rendering).
...
PR4, PR5, and PR6 are merged per coordinating maintainer confirmation (observed merge `c892613`). The current branch is `feat/visible-nodule-focus`; its new PR has not been created. The coordinating maintainer owns all Git operations, the whole-implementation GLM privacy gate, and subsequent submission; no new privacy or CI pass is claimed. Use the single primary local worktree and one application server; do not create parallel checkouts or manage the caller's proxy as part of documentation work. Code-publication approval never includes patient-derived assets.
