...
export function drawSliceView(target, source, { flipRows = false } = {}) {
...
export function drawSliceOverlay(
  canvas,
  { sourceWidth, sourceHeight, pixel, annotations, axis, index, affineRas, shape, flipRows = false },
) {
  ...
  for (const annotation of annotations || []) {
    ...
    const row = nativeRowToDisplay(projected.row, canvas.height, flipRows);
    ...
    context.beginPath();
    context.ellipse(projected.col, row, rx, ry, 0, 0, Math.PI * 2);
    context.stroke();
  }
  if (pixel) {
    const row = nativeRowToDisplay(pixel.row, canvas.height, flipRows);
    ...
    context.lineTo(canvas.width, row + 0.5);
    context.stroke();
    context.fillStyle = "#f3ead8";
    context.fillRect(pixel.col - 2, row - 2, 5, 5);
  }
}
