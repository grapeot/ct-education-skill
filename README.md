...
Both panels use source geometry in RAS millimeters mapped to Blender meters without axis permutation. The first 28 seconds, including the 26-28 second parity proof, remain unchanged. From 28-36 seconds, N1 persists in both panels with exterior corner brackets, an offset label, and a leader to keep the focal pixels clear. The left renders a physically mapped native-HU region of interest (ROI), three local planes, and an optional exploratory isodensity surface; overview anatomy, including airways, is hidden. Its camera locks on N1, settles framing by 30 seconds, orbits 50 degrees from 30-35.5 seconds, and holds through 36 seconds. Right native CT pixels, crop, and ruler stay fixed after 30 seconds; the left projected slice-plane ruler persists. No annotation means source-only fallback. This film-only postprocessing leaves native HU and interactive annotation locators unchanged.

The optional density surface is not an accepted nodule boundary. Threshold sensitivity or lower-threshold leakage keeps the strict `mesh=None` and `report.status=localized_region_boundary_unverified`, even if a bounded selected component passes the separate display checks. Unsafe selected components produce no `density_mesh`, falling back to native-HU planes and a wire locator. The on-frame caption reads "CT density surface at ... HU; not a verified nodule boundary.", using the runtime threshold. See the [extraction contract](docs/rfc.md#cinematic-rendering); no geometry is fabricated to force a surface.

The latest authorized film completed at 36 seconds, 1920x1080, 24 fps: 864 Blender frames, with unchanged pre-28-second imagery retained, no frame interpolation, and no audio. The maintainer reports full validation and decoding passed. The run contains `film.mp4`, PNGs, textures, state, logs, validation records, external `candidate/` evidence, and a packed `master.blend`. All are private derivatives; ROI arrays, masks, threshold trials, provenance, and the packed scene must never enter the repository. Rendering does not expose any file to the viewer.
