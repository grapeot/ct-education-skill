...
- Independently rerun by the documentation reviewer on 2026-09-14 after the persistent-N1/density-surface revision: **all 136 Python tests passed with the opt-in ffmpeg smoke enabled; all 53 frontend tests passed**, with zero skips. The production build remains a prior maintainer-reported pass, not a build rerun in this documentation pass.
- Python coverage comprises 32 pipeline, 10 repository hygiene, 28 browser-video, 20 video HTTP, 9 two-video catalog, 28 cinematic, and 9 candidate-geometry tests. Default discovery skips the one opt-in smoke test; the enabled run executes all 136.
...
- Required `master` check: `scaffold-hygiene`. Current CI runs default Python discovery, `npm test`, and the generic frontend build, but not the opt-in encoder smoke or Playwright browser installation. It does not deploy or upload artifacts. No new remote CI result for this branch is claimed.
- Maintainer-confirmed release status: PR4, PR5, and PR6 are merged (observed merge `c892613`). No new PR has been created for `feat/visible-nodule-focus`. The whole-implementation GLM privacy gate and subsequent Git/submission remain with the coordinating maintainer; earlier reviews do not establish a new privacy or CI pass.
- Prior verification checked CLI help for all five subcommands. The earlier generic external annotation example was accepted on synthetic geometry with generic output IDs; this pass changed no CLI examples or options.
- This documentation pass changed only the eight owned Markdown files, using a Cursor rough draft of generic public-safe facts followed by source review and surgical edits. It independently reran the Python and frontend suites; all 10 scaffold/hygiene tests passed with no public-file hygiene findings. It performed no Git commands, source/test edits, renders, or browser QA. Current publication review remains with the coordinating maintainer.
...
`tests/test_cinematic.py` contains 28 synthetic unit tests. They cover native/cropped pixel-center mapping and read-only HU, asymmetric non-axial orientation, shear rejection, ruler selection, option/output boundaries, and CLI defaults/errors. Timeline checks cover all 864 frame states and shot boundaries, monotone Hermite orbit knots and shared velocities, a full unwrapped turn, fast-slow-fast contrast, staged stack entry/restoration, sweep interiors and holds, the 26-28 second parity hold, branch emphasis, and serializable candidate crops. Local assertions cover depth-view state without image-parity claims, hidden overview anatomy, persistent N1/marker/ruler state across every 28-36 second frame, monotone orbit progress with the 35.5-36 second hold, and unchanged right CT pixels/crop/ruler after 30 seconds. These tests do not run Blender or certify rendered appearance or label occlusion.

`tests/test_candidate_geometry.py` adds 9 synthetic tests for source-HU preservation and affine mapping, air-seed/edge-connected rejection, disconnected-component exclusion, invalid seeds/options, and strict rejection of lower-threshold leakage and threshold-sensitive boundaries. A separate exploratory `density_mesh` may pass basic selected-component checks while `mesh=None` and `report.status=localized_region_boundary_unverified` remain. The combined cinematic/candidate subset grew from 35 to 37 tests with two additions: vertices follow the exact native-HU isovalue rather than a sphere prior, and the caption uses the runtime threshold without promoting boundary status. These are synthetic checks, not verified nodule segmentation.

The coordinating maintainer reports the latest authorized film completed at **36 seconds, 1920x1080, 24 fps, 864 Blender frames**, with full validation, MP4 probe, and full ffmpeg decode passed. Pre-28-second imagery is unchanged; the final shots retain N1 in both panels, a local native-HU ROI with optional exploratory density surface, and a 50-degree left camera orbit locked to N1. Right CT stays fixed after 30 seconds. No temporal interpolation, audio, source-HU changes, or fabricated geometry were introduced. The earlier airway-only ending is superseded; earlier paired-image and browser checks retain their historical scope, not new appearance or playback acceptance. No case coordinates, image values, identifiers, paths, or exact study-derived metrics are published here; technical completion is not continuous human aesthetic certification.
...
Automated scanning cannot recognize every identifying case fact. Keep all runtime inputs and derivatives outside the repository and public history, including external `candidate/` ROI arrays, masks, threshold trials, validation, and provenance. Selected-case parameters, voxel positions, volumes, and ratios must not enter public results. Public outcomes must remain generic even when local testing has separate authorization to use private inputs. Default to no uploads; any GPT review requires explicit per-case approval of material, purpose, and destination under [the privacy gate](../AGENTS.md#privacy-gate). General testing or release permission does not authorize transmission or public disclosure.
...
Earlier independent GPT QA in actual Chrome through authorized caller-managed private HTTPS passed desktop, phone portrait, and landscape checks: both version controls, the retained 30-second demo and then-current 36-second film durations, playback, seeking, matching direct-play/download links, attachment download, no MP4 request before click, close-time pause/source removal, and pause/reset behavior when the video changes. Prior checks also covered focus restoration, Escape from focused controls, and landscape footer fit. These are bounded historical functional checks, not new playback acceptance, iPhone Safari certification, comprehensive cross-browser coverage, clinical validation, or continuous human aesthetic acceptance. This documentation worker did not repeat browser QA.
