...
- Cinematic export: `render-cinematic` implements the [36-second design](video_design.md) independently of browser capture. RAS-mm to Blender-meter mapping and imagery before 28 seconds, including the 26-28 second parity proof, are unchanged. From 28-36 seconds, N1 brackets and leaders persist in both panels. The left renders a physical native-HU ROI with optional exploratory density surface or native-HU planes and wire-locator fallback; overview anatomy is hidden. The camera stays locked to N1 through a 50-degree orbit from 30-35.5 seconds, then holds. Right CT and its ruler stay fixed after 30 seconds; the qualified left ruler persists. Native HU and interactive annotation locators remain unchanged by film-only postprocessing.
- Candidate display boundary: Strict `mesh=None` and `report.status=localized_region_boundary_unverified` remain on threshold sensitivity or lower-threshold leakage. A separate `density_mesh` requires a bounded non-edge selected component passing basic checks and the exact runtime native-HU isovalue. It is not an accepted nodule boundary; its on-frame caption reads "CT density surface at ... HU; not a verified nodule boundary." Unsafe selected components yield no density mesh, not fabricated geometry. See the [extraction contract](rfc.md#cinematic-rendering).
...
Responsive controls and the Aurora default are implemented. Earlier independent GPT QA in actual Chrome through authorized private HTTPS passed desktop, phone portrait, and landscape functional checks for both then-current videos. Small mobile inline playback and some ruler-label offsets remain accepted P2 polish items, not launch blockers; fullscreen and the direct-play link are available. These bounded historical checks do not establish new film playback acceptance, high-fidelity masks, continuous human aesthetic acceptance, iPhone Safari certification, or comprehensive cross-browser coverage; see [test status](test.md#visual-qa-and-regressions).
...
All runtime assets and case facts remain private and external, including sanitized derivatives. Candidate ROI arrays, masks, threshold trials, validation, and provenance stay under external `candidate/` or other private runtime directories; selected-case parameters, voxel positions, volumes, and ratios are never public examples. No raw patient data, derivatives, identifiers, private paths, or case facts belong in public repositories/history, assets, docs, PRs, issues, CI, or logs. Default to no uploads. GPT review requires explicit per-case user approval of material, purpose, and destination; build/test/release authorization is not upload consent. Agents cannot infer or self-grant it, and must stop before transmission if its scope is unspecified. This exception approves neither other providers nor public disclosure; the application has no upload route. See [privacy rules](../AGENTS.md#privacy-gate).
...
- Implemented baseline: all five CLI commands, geometry-preserving HU, reduced-grid candidates, local viewer, private output, and opt-in two-video playback/download. Independent reruns passed all 136 Python tests with encoder smoke and 53 frontend tests with zero skips; the production build remains a maintainer-reported pass. Details are in [test status](test.md).
- Verified use: the maintainer reports the latest 36-second, 1920x1080, 24 fps film completed with 864 frames and full validation/decode, without interpolation or audio. Earlier independent GPT Chrome QA passed scoped two-video functionality; it is not new browser or aesthetic acceptance of the persistent-N1 ending. Mask quality and continuous human aesthetic acceptance remain uncertified.
