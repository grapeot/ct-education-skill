...
- Separate cinematic run: `state.json`, copied `generate_scene.py`, `meshes/`, `textures/`, `left/` PNGs and projection metadata, composed `frames/`, packed `master.blend`, `benchmark.json`, `film.mp4`, logs, `probe.json`, and `validation.json`. When annotated, external `candidate/` holds ROI HU, selected and trial masks, optional strict/density meshes, three local images, `validation.json`, and `density_display.json`. All are private, including the patient-derived packed scene. The caller may copy only the final MP4 into the viewer workspace after explicit approval; nothing is copied or exposed automatically.
...
`render-cinematic` uses `cinematic.py` for source-plane state and composition, `candidate_geometry.py` for bounded native-HU extraction, and `cinematic_blender.py` for deterministic EEVEE scene generation. It requires Blender, ffmpeg, and ffprobe, not a browser or application server. `--workspace` and `--output` are required. Defaults are `--blender blender --start 0 --duration 36 --fps 24 --width 1280 --height 720 --samples 16 --budget 600`. The output is a new disjoint external directory with an existing parent. See the [option table](../skills/ct_education.md#cinematic-rendering) for exact limits; no CLI resume flag is provided.

Native arrays remain `[k,j,i]`. The shared source-plane state records origin, pixel-center basis, crop, affine, window, and image identity. Non-axial rows become superior-up when required by the affine; an in-plane shear fails closed. RAS millimeters map to Blender Z-up meters as `0.001 * (ras - stored_center_ras)`, without axis permutation or sign changes. The main left HU texture and right image share a body-footprint display mask, not reviewed segmentation. The final left ROI instead uses three separately cropped native source-grid planes with all ROI pixels retained; these are not anatomical world-axis reformats for oblique input. Mesh clipping and display offsets never change source sampling or rest coordinates.

The left ruler is a true plane-local RAS-mm segment projected through the current camera; the right ruler uses physical crop extent and the final resized image rectangle, excluding letterboxing. Both use readable 1/2/5-series lengths. At an exact orthographic face-on pose, composition checks camera basis agreement and ruler-length parity within one pixel. The later tilted orthographic local view is not an image-parity pose. Both rulers persist through the ending; the left remains qualified as "in slice plane", not a global 3D scale or lesion diameter. Neither a locator nor a display surface supplies a clinical measurement.
...
From 28-36 seconds, `depth_view` hides all overview anatomy layers and renders a physically mapped native-HU ROI around the first supplied candidate, labeled N1. Exterior corner brackets, an offset label, and a leader persist in both panels to keep the focal pixels clear. Left framing settles at 30 seconds, then a 50-degree orthographic camera orbit runs through 35.5 seconds with its target locked to N1, not airway bounds; the final half-second holds. Right CT pixels, crop, and ruler stay fixed after 30 seconds. The local planes are faint when a shaded density surface is available; otherwise they remain visible with a wire locator. No supplied candidate means source-only fallback, not an invented finding. This film-only postprocessing leaves native HU, labels, and interactive annotation locators unchanged.

`extract_candidate` selects a seed-connected 26-neighbor component within a bounded physical native-HU ROI, without a sphere prior, morphology, geometry smoothing, or fabricated caps. Strict `mesh` remains `None` and `report.status` remains `localized_region_boundary_unverified` if the selected component fails basic checks, a lower threshold connects to the ROI edge, or the boundary is threshold-sensitive. Passing strict gates yields only `approximate_surface`, never a reviewed or clinical boundary. Separately, `density_mesh` may exist only for a non-edge selected component passing basic evidence, volume, and elongation checks. Marching cubes uses the exact runtime HU threshold on the selected native-HU component field, not a binary-mask surface. Unsafe selected components produce no density mesh and fall back to three native-HU planes with a wire locator. No gate is relaxed to force geometry.

`density_display.representation_type` is `exploratory_isodensity`, with `verified_nodule_boundary=false` and all boundary warnings retained. Its on-frame caption is "CT density surface at ... HU; not a verified nodule boundary.", replacing the ellipsis with the runtime threshold. Display availability never promotes the strict boundary status. ROI arrays, masks, threshold trials, meshes, and provenance remain in external private `candidate/` and run records; selected-case settings, voxel positions, volumes, and ratios never enter public documentation or assets.
